...
void
gertboardAnalogWrite(const int chan, const int value)
{
    uint8_t spiData[2];
...
int
gertboardAnalogRead(const int chan)
{
    uint8_t spiData[2];
...
static int
myAnalogRead(struct wiringPiNodeStruct *node, const int chan)
{
    return gertboardAnalogRead(chan - node->pinBase);
}

static void
myAnalogWrite(struct wiringPiNodeStruct *node, const int chan, const int value)
{
...
int
gertboardAnalogSetup(const int pinBase)
{
