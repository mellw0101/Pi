 ...
 *	sampled on the trailing edge of the last clock, so it's valid
 *immediately.
 *********************************************************************************
 */
...
static void
ds1302regWrite(const int reg, const unsigned int data)
{
    digitalWrite(sPin, HIGH);
    delayMicroseconds(1);
    dsShiftOut(reg);
...
unsigned int
ds1302rtcRead(const int reg)
{
    return ds1302regRead(0x81 | ((reg & 0x1F) << 1));
}

void
ds1302rtcWrite(int reg, unsigned int data)
{
...
unsigned int
ds1302ramRead(const int addr)
{
    return ds1302regRead(0xC1 | ((addr & 0x1F) << 1));
}

void
ds1302ramWrite(const int addr, const unsigned int data)
{
...
void
ds1302clockRead(int clockData[8])
{
    int          i;
...
void
ds1302clockWrite(const int clockData[8])
{
    int          i;
...
void
ds1302trickleCharge(const int diodes, const int resistors)
{
...
void
ds1302setup(const int clockPin, const int dataPin, const int csPin)
{
    dPin = dataPin;
    cPin = clockPin;
    sPin = csPin;
