...
static void
sendData(const int data, const int chip)
{
    digitalWrite(chip, 0);
...
static void
sendCommand(const int command, const int chip)
{
    digitalWrite(RS, 0);
...
static void
setCol(int col, const int chip)
{
    sendCommand(0x40 | (col & 0x3F), chip);
}

static void
setLine(int line, const int chip)
{
...
void
lcd128x64setOrigin(int x, int y)
{
    xOrigin = x;
    yOrigin = y;
}
...
void
lcd128x64setOrientation(int orientation)
{
...
void
lcd128x64orientCoordinates(int *x, int *y)
{
    /* register  */ int tmp;
...
void
lcd128x64getScreenSize(int *x, int *y)
{
    *x = maxX;
    *y = maxY;
}
...
void
lcd128x64point(int x, int y, int colour)
{
    lastX = x;
    lastY = y;
...
void
lcd128x64line(int x0, int y0, int x1, int y1, int colour)
{
    int dx, dy;
    int sx, sy;
    int err, e2;
...
void
lcd128x64lineTo(int x, int y, int colour)
{
...
void
lcd128x64rectangle(int x1, int y1, int x2, int y2, int colour, int filled)
{
    /* register */ int x;
...
void
lcd128x64circle(int x, int y, int r, int colour, int filled)
{
    int ddF_x = 1;
    int ddF_y = -2 * r;
...
static void
plot4ellipsePoints(int cx, int cy, int x, int y, int colour, int filled)
{
    if (filled)
    {
...
void
lcd128x64ellipse(int cx, int cy, int xRadius, int yRadius, int colour, int filled)
{
    int x, y;
...
void
lcd128x64putchar(int x, int y, int c, int bgCol, int fgCol)
{
    int y1, y2;
...
void
lcd128x64puts(int x, int y, const char *str, int bgCol, int fgCol)
{
    int c, mx, my;
