...
static const uint16_t dataRates[8] = {CONFIG_DR_8SPS,   CONFIG_DR_16SPS,  CONFIG_DR_32SPS, CONFIG_DR_64SPS,
                                      CONFIG_DR_128SPS, CONFIG_DR_475SPS, CONFIG_DR_860SPS};
...
static void
myDigitalWrite(struct wiringPiNodeStruct *node, int pin, int data)
{
    int chan = pin - node->pinBase;
    chan &= 3;
...
static void
myAnalogWrite(struct wiringPiNodeStruct *node, int pin, int data)
{
    int     chan = pin - node->pinBase;
    int     reg;
    int16_t ndata;
...
int
ads1115Setup(const int pinBase, int i2cAddr)
{
