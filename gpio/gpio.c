...
static void
doI2Cdetect(const char *progName)
{
...
void
SYSFS_DEPRECATED(const char *progName)
{
    fprintf(stderr,
            "%s: GPIO Sysfs Interface for Userspace is deprecated "
            "(https://www.kernel.org/doc/Documentation/gpio/sysfs.txt).\n Function is now useless and empty.\n\n",
            progName);
}

void
LOAD_DEPRECATED(const char *progName)
{
    fprintf(stderr,
            "%s: load/unload modules is deprecated. You need to run the raspi-config program (as root) and select the "
            "interface option (SPI or I2C) that you wish to de-/activate.\n\n",
            progName);
}
...
void
printgpioflush(const char *text)
{
    if (gpioDebug)
    {
        printf("%s", text);
...
static void
doPadDrivePin(int argc, char *argv[])
{
...
static void
doWriteByte(int argc, char *argv[])
{
    int val;
...
static void
doReadByte(int argc, char *argv[], int printHex)
{
    int val;
...
doVersion(char *argv[])
{
    ...
    printf("  Type: %s, Revision: %s, Memory: %dMB, Maker: %s %s\n", piModelNames[model], piRevisionNames[rev],
           piMemorySize[mem], piMakerNames[maker], warranty ? "[Out of Warranty]" : "");
